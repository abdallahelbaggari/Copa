...
function piLogin(){
  ...
  /* Re-attempt Pi.init in case SDK loaded after page ready */
  if(!PI_READY) _initPiSDK();

  /* Still not ready — try one more time after short delay */
  if(!PI_READY){
...
function _doAuthenticate(btn, btxt){
  /* Call Pi.authenticate — wallet_address scope triggers biometric in Pi Browser */
  try {
    Pi.authenticate(
    ...
    .catch(function(err){
      ...
      if(low.indexOf('pending')!== -1||low.indexOf('incomplete')!==-1){
        ...
        setLS('Cancelled — tap Sign in to try again', 'err');
      } else if(low.indexOf('network')!==-1||low.indexOf('fetch')!==-1){
        setLS('Network error — check connection', 'err');
      } else {
        setLS('Sign in failed: '+msg.substring(0,50), 'err');
      }
      if(btn)  btn.disabled = false;
      if(btxt) btxt.textContent = t('signin');
    });
    ...
    setLS('Pi SDK error — reload and try again', 'err');
    if(btn)  btn.disabled = false;
    if(btxt) btxt.textContent = t('signin');
  }
}
...
function renderLeaderboard(){
  ...
  el.innerHTML=meRow+lb.map(function(u,i){
    ...
    return '<div class="lb-row">'
      ...
      +'<div><div class="lb-pts">'+u.pts+'pts</div><div cla
